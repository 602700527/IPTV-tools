#!/usr/bin/env python3
"""Generate new generateStaticHomepage for seo-handler.js"""
import re
...
# First escape backslashes, then backticks, then ${ (order matters)
css_escaped = css.replace('\\', '\\\\').replace('`', '\\`').replace('${', '\\${')

print(f"CSS: {len(css)} chars, escaped: {len(css_escaped)}")
...
export async function generateStaticHomepage(request, env, page = 1) {
  ...
  // 原版完整 CSS
  const ORIGINAL_CSS = `''' + css_escaped + '''`;
  ...
  const sectionLabel = safePage === 1
    ? `All Channels <span style="color:rgba(255,255,255,0.5);font-weight:400;font-size:14px;margin-left:12px">${totalChannels.toLocaleString()} channels</span>`
    : `Page ${safePage} of ${totalPages} <span style="color:rgba(255,255,255,0.5);font-weight:400;font-size:14px;margin-left:12px">${totalChannels.toLocaleString()} channels</span>`;
...
print(f"Old function: chars {func_start_idx} to {func_end_idx} ({func_end_idx - func_start_idx} chars)")

new_seo_content = seo_content[:func_start_idx] + new_func + '\n\n' + seo_content[func_end_idx:]

with open(r'C:\Users\60270\Desktop\cfworker2\handlers\seo-handler.js', 'w', encoding='utf-8') as f:
    f.write(new_seo_content)

print(f"New file written. Total: {len(new_seo_content)} chars")
print("Done!")
