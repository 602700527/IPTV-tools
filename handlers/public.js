...
// [删除的代码 / DELETED CODE - 2026-08-30]
// 原 handlePublicChannels 函数（408 行）已在 commit 5549ba9 中删除
// （原 commit hash 是 164b775，amend 后变成 5549ba9）
// 删除原因：未被任何代码路径调用（dead code），但每次调用会触发 3 次 D1 查询：
//   1. SELECT COUNT(*) FROM channels
//   2. SELECT ... FROM channels WHERE ... LIMIT ? OFFSET ?
//   3. SELECT DISTINCT group_title FROM channels
// 留作隐患：若未来被误用会瞬间爆 D1 配额。
// 如需恢复：git show 5549ba9^:handlers/public.js | sed -n '505,912p'
// 替代方案：使用 handlers/admin.js 的 'channels' case（已走 KV 缓存）
