#!/usr/bin/env python3
"""Generate redesigned generateSEOHomepage - version 2"""
import re
...
# Escape for JS: backslash first, then backtick, then ${
def js_escape(s):
    return s.replace('\\', '\\\\').replace('`', '\\`').replace('${', '\\${')

css_escaped = js_escape(css)

# Build the full new function text directly
# Using explicit string concatenation to avoid Python indent issues
nl = '\n'
...
func_lines = [
    ...
    '            <option value="${origin}/?lang=zh-CN">\\u4e2d\\u6587</option>',
    '          </select>',
    '        </div>',
    '      </div>',
    '    </header>`;',
    ...
    '        </div>',
    '        <div class="channel-group">${escapeHtml(ch.group_title || \'\'一出\')}</div>',
    '      </div>',
    '    </div>`;',
    '  }).join(\'\');',
    '',
...
print(f"Replacing chars {func_start} to {func_end}")
