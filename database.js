...
export async function createTables(env) {
  ...
  // ponytail: 默认种子（payment_methods / mall_settings）已抽出到 seedDefaultRows(env)，只在 admin init/migrate 调用一次。
  // 原因：每个新 isolate 跑一次 createTables 会触发 6+2 条 COUNT 查询 — D1 不该被打。
...
// ponytail: 默认种子（payment_methods / mall_settings）。从 createTables 抽出。
// 只在 admin /admin/init 和 /admin/migrate 调用一次，不放进 per-fetch 路径。
...
export async function seedDefaultRows(env) {
  ...
  // ponytail: 清理历史软禁用（is_active=0）但没真正删除的频道数据。
  // 现在禁用源会直接 DELETE channels，这个一次性清理抹掉历史脏数据。
  try {
    const cleanupResult = await db.prepare('DELETE FROM channels WHERE is_active = 0').run();
    const cleaned = cleanupResult.meta?.changes || 0;
    if (cleaned > 0) {
      console.log('Database: Cleaned ' + cleaned + ' stale inactive channels (is_active=0)');
    }
  } catch (e) {
    console.error('Database: Failed to cleanup inactive channels:', e);
  }

  // ponytail: 清理 source 已禁用但 channel 还在（软禁用时期的 orphan rows）
  // 现在禁用源会直接 DELETE channels，这个一次性清理抹掉历史脏数据。
  try {
    const orphanResult = await db.prepare(
      "DELETE FROM channels WHERE source_id IN (SELECT id FROM sources WHERE is_active = 0)"
    ).run();
    const orphans = orphanResult.meta?.changes || 0;
    if (orphans > 0) {
      console.log('Database: Cleaned ' + orphans + ' orphan channels (source is_active=0)');
    }
  } catch (e) {
    console.error('Database: Failed to cleanup orphan channels:', e);
  }
}
...
export async function parseM3UContent(content, sourceId, filter = {}) {
  ...
  for (const block of blocks) {
    ...
    // 注：关键词必须是**非空字符串**。空字符串 ''.includes('') 永远为 true，
    // 会让每个频道命中所有规则，导致 type 字段塞满所有分类。
    // 历史 bug：commit 7e1963c 不小心把所有中文字符串清空，导致全频道同标签。
    ...
    for (const rule of CHANNEL_TYPE_KEYWORDS) {
      // 防御：跳过空关键词数组（如果以后有人误清空，不至于再次全员命中）
      if (!rule.keywords || rule.keywords.length === 0) continue;
      if (rule.keywords.some(kw => kw && channelName.includes(kw))) {
