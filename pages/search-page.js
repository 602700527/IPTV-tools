...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .channel-poster .placeholder { width: 72px; height: 48px; opacity: 0.4; flex-shrink: 0; }
      ...
      try {
        ...
        if (results.length > 0) {
          resultsContainer.innerHTML = '<div class="channel-grid">' + results.map(ch => {
            const logoHtml = ch.logo ? '<img src="' + escapeHtml(ch.logo) + '" alt="' + escapeHtml(ch.name) + '">' : '<div class="placeholder" style="width:72px;height:48px;flex-shrink:0"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="100%" height="100%"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg></div>';
